var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
        extent: [-13620709.091751, 6069583.557622, -13610104.514652, 6075171.895232], maxZoom: 28, minZoom: 1
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([-13620709.091751, 6069583.557622, -13610104.514652, 6075171.895232], map.getSize());
...
function onPointerMove(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") == undefined)) {
            ...
            // Skip hover for parcels layer - performance fix for large layers
            var layerTitle = layer.get('title') || '';
            var popupLayerTitle = layer.get('popuplayertitle') || '';
            
            if (layerTitle.toLowerCase().includes('parcel') || popupLayerTitle.toLowerCase().includes('parcel')) {
                return; // Skip hover processing for parcels
            }
	...
	if (doHighlight) {
        // Also skip highlighting for parcels - performance fix
        if (currentLayer) {
            var layerTitle = currentLayer.get('title') || '';
            var popupLayerTitle = currentLayer.get('popuplayertitle') || '';
            
            if (layerTitle.toLowerCase().includes('parcel') || popupLayerTitle.toLowerCase().includes('parcel')) {
                return; // Skip highlighting for parcels
            }
        }
...
function onSingleClickFeatures(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            ...
            // Force popups for parcels layer regardless of field configuration
            var layerTitle = layer.get('title') || '';
            var popupLayerTitle = layer.get('popuplayertitle') || '';
            
if (layerTitle.toLowerCase().includes('parcel') || popupLayerTitle.toLowerCase().includes('parcel')) {
    doPopup = true;
} else if (layerTitle.toLowerCase().includes('street') || popupLayerTitle.toLowerCase().includes('street')) {
    doPopup = true;
} else {
...
// Custom formatting for streets layer
if (layerTitle.toLowerCase().includes('street') || popupLayerTitle.toLowerCase().includes('street')) {
    var streetName = currentFeature.get('STREET_NAM') || 'Unknown Street';
    var cfcc = currentFeature.get('CFCC') || '';
    var fedRoadClass = currentFeature.get('FedRoadCla') || '';
    var mltRoadClass = currentFeature.get('MLTRoadCla') || '';
    var shapeLength = currentFeature.get('Shape_Leng') || 0;
    var lengthFeet = Math.round(shapeLength * 3.28084);
    
    popupText += '<tr><th colspan="2"><strong>' + streetName + '</strong></th></tr>';
    popupText += '<tr><th>CFCC:</th><td>' + cfcc + '</td></tr>';
    popupText += '<tr><th>Fed Road Class:</th><td>' + fedRoadClass + '</td></tr>';
    popupText += '<tr><th>MLT Class:</th><td>' + mltRoadClass + '</td></tr>';
    popupText += '<tr><th>Length:</th><td>' + lengthFeet + ' feet</td></tr>';
} else {
    popupText += createPopupField(currentFeature, currentFeatureKeys, layer);
}
                        popupText += '</table></li>';    
